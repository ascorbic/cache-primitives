...
import { createWriteHandler } from "../../src/handlers.ts";
import {
	defaultGetCacheKey,
	parseCacheControl,
	parseCacheTags,
...
Deno.test("Security - Cache pollution via tag injection", async () => {
	...
	// First, cache a legitimate response
	...
	await writeHandler(request1, legitimateResponse);
	...
	await writeHandler(request2, maliciousResponse);
