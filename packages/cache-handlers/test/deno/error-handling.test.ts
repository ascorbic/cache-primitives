...
Deno.test("Error Handling - ReadHandler with cache match failure", async () => {
	...
	const readHandler = createReadHandler({ cache: failingCache });

	const request = new Request("https://example.com/api/users");

	// Should handle cache match failure gracefully
	await assertRejects(() => readHandler(request), Error, "Cache match failed");
	await caches.delete("test");
});

Deno.test("Error Handling - WriteHandler with cache put failure", async () => {
	const failingCache = new FailingCache("put");
	const writeHandler = createWriteHandler({ cache: failingCache });
...
Deno.test(
	...
	async () => {
		...
		if (cached) await cached.text(); // Clean up resource
		await caches.delete("test");
	},
);
...
Deno.test(
	"Error Handling - MiddlewareHandler with next() throwing error",
	async () => {
		const cache = await caches.open("test");
		const middlewareHandler = createMiddlewareHandler({ cache });
		...
		// Should propagate the error from next()
		await assertRejects(
			() => middlewareHandler(request, next),
			Error,
...
Deno.test(
	"Error Handling - MiddlewareHandler with cache read failure",
	async () => {
		const failingCache = new FailingCache("match");
		const middlewareHandler = createMiddlewareHandler({ cache: failingCache });

		const request = new Request("https://example.com/api/users");
		const next = () => Promise.resolve(new Response("fresh data"));

		// Should handle cache read failure and still call next()
		await assertRejects(
			() => middlewareHandler(request, next),
			Error,
			"Cache match failed",
